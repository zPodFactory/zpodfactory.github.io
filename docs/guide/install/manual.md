...
!!! warning "Deprecated — developers only"
    This installation path is not recommended. Use the [zPodFactory appliance](index.md) instead.
...
We recommend starting with the [zPodFactory appliance](index.md#using-the-zpodfactory-appliance-highly-recommended) for any development work. If you need a standalone Linux base, the legacy [zBox](https://cloud.tsugliani.fr/ova/zbox-12.9.ova) appliance was the previous development platform; new zPods use **`zcore`** as the mandatory core component (see [zcore migration](../admin/zcore-migration.md)).
...
Install [uv](https://docs.astral.sh/uv/) (manages Python versions and dependencies):
